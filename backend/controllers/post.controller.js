...
export const getUserPosts = async(req, res) => {
    ...
    try {
        ...
        const posts = await Post.find({user: user._id, postType: "regular"})
        .sort({createdAt: -1})
        .populate({
            path: "user",
            select: "-password",
...
export const getCategoryPostsByUser = async (req, res) => {
    try {
        ...
        const validCategories = ['workout', 'mealprep'];
        ...
        console.log("Error in getCatergoryByUser controller: ", error);
        res.status(500).json({error: "Internal server error"});
    }
};
